...
def getActor(response):
    if re.search(r'<a href="/star/\S+">(\S+)</a> &nbsp;', response):
        return str(
            re.findall(r'<a href="/star/\S+">(\S+)</a> &nbsp;',
                       response)).strip(" [',']").replace('\'', '')
    elif re.search(r'<a href="/heyzo_star/\S+">(\S+)</a> &nbsp;', response):
        return str(
            re.findall(r'<a href="/heyzo_star/\S+">(\S+)</a> &nbsp;',
                       response)).strip(" [',']").replace('\'', '')
    else:
        return str(re.findall(r'<b>出演者</b>: ([^<]+) &nbsp; <br>',
                              response)).strip(" [',']").replace('\'', '')


def getStudio(response):
    return str(re.findall(r'<a href="/company/\S+">(\S+)</a>',
                          response)).strip(" ['']")


def getRuntime(response):
    return str(re.findall(r'<b>収録時間</b>: (\d+) \S+<br>',
                          response)).strip(" ['']")


def getSeries(response):
    return str(
        re.findall(r'<b>系列</b>: <a href="/series/\S+">(\S+)</a>',
                   response)).strip(" ['']")


def getWebsite(detail_page):
    return 'https:' + detail_page.xpath(
        '//a[contains(text(),"简体中文")]/@href')[0]


def getNum(response):
    return str(re.findall(r'<b>品番</b>: (\S+)<br>',
...
def getScore(response):
    if re.search(r'<b>平均評価</b>: <img data-original="/img/(\d+).gif" />',
        ...
        return str(re.findall(r'<b>平均評価</b>: ([^<]+)<br>',
                              response)).strip(" [',']").replace('\'', '')
...
def getRelease(response):
    return str(re.findall(r'<b>配信開始日</b>: (\d+-\d+-\d+)<br>',
...
def getCoverSmall(detail_page):
    return str(
        detail_page.xpath(
            "//div[@class='panel-body']/div[@class='row'][1]/div[@class='col-md-3']/img["
...
def getOutline(detail_page):
    return str(
        detail_page.xpath(
            '/html/body/div[2]/div[1]/div[1]/div[2]/div[3]/div/text()')).strip(
                " ['']")
