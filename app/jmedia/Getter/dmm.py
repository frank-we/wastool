...
def getNum(text):
    ...
    try:
        result = html.xpath(
            "//td[contains(text(),'品番：')]/following-sibling::td/a/text()")[0]
    except:
        result = html.xpath(
            "//td[contains(text(),'品番：')]/following-sibling::td/text()")[0]
    return result
...
def getOutline(text):
    ...
    try:
        result = str(
            html.xpath("//div[@class='mg-b20 lh4']/text()")[0]).replace(
                "\n", "")
        if result == "":
            result = str(
                html.xpath("//div[@class='mg-b20 lh4']//p/text()")[0]).replace(
                    "\n", "")
    except:
        # (TODO) handle more edge case
        # print(html)
...
def getSeries(text):
    try:
        html = etree.fromstring(
            text, etree.HTMLParser())  # //table/tr[1]/td[1]/text()
        try:
            result = html.xpath(
...
def main(number, appoint_url=''):
    ...
    fanza_search_number = re.sub(r"[^0-9a-zA-Z_]", "",
                                 fanza_search_number).lower()
    ...
    try:
        ...
        dic = {
            ...
            "director":
            getDirector(htmlcode) if "anime" not in chosen_url else "",
            ...
            "year":
            getYear(release),  # str(re.search('\d{4}',getRelease(a)).group()),
            "actor_photo": "",
            "website": chosen_url,
            "source": "fanza.py",
