...
def getTitle(a):
    ...
    result = str(html.xpath(
        "//span[@id='program_detail_title']/text()")).strip(" ['']").replace(
            "'", '')
    return result
...
def getActor(
    ...
    html = etree.fromstring(a, etree.HTMLParser())
    result = str(html.xpath("//li[@class='credit-links']/a/text()")).strip(
        " ['']").replace("'", '')
    return result
...
def getStudio(a):
    html = etree.fromstring(a,
                            etree.HTMLParser())  # //table/tr[1]/td[1]/text()
    result = str(html.xpath(
        "//span[@id='program_detail_maker_name']/text()")).strip(" ['']")
    return result
...
def getRuntime(a):
    ...
    try:
        result = str(
            html.xpath("//span[contains(text(),'収録時間')]/parent::li/text()"))
...
def getSeries(a):
    ...
    result = str(
        html.xpath(
            "//span[contains(text(),'シリーズ')]/following-sibling::a/span/text()")
    ).strip(" ['']").replace('\'', '')
    return result
...
def getRelease(a):
    ...
    try:
        result = str(
            html.xpath("//span[contains(text(),'発売日')]/parent::li/text()"))
        if re.search(r'\d{4}/\d{2}/\d{2}', result):
            result = re.findall(r'\d{4}/\d{2}/\d{2}',
                                result)[0].replace('/', '-')
    except:
        result = ''
    return result
...
def getTag(a):
    html = etree.fromstring(a,
                            etree.HTMLParser())  # //table/tr[1]/td[1]/text()
    result1 = str(
        html.xpath("//a[@class='genre']/text()")).strip(" ['']").replace(
            '\'', '').replace('\\t', '').replace('\\n', '')
...
def getCover(htmlcode):
    ...
    result = 'https:' + str(
        html.xpath("//div[@class='photo']/p[@class='tn']/a/@href")).strip(
            " ['']").replace('\'', '')
    return result
...
def getDirector(a):
    html = etree.fromstring(a, etree.HTMLParser())
    result1 = str(
        html.xpath("//span[@id='program_detail_director']/text()")).strip(
            " ['']").replace('\\t', '').replace('\\n', '')
    return result1
...
def getOutline(htmlcode):
    html = etree.fromstring(htmlcode, etree.HTMLParser())
    result = str(
        html.xpath("//p[@class='lead']/text()")).strip(" ['']").replace(
            '\\n', '').replace('\n', '')
    return result
...
def getScore(htmlcode):
    html = etree.fromstring(htmlcode, etree.HTMLParser())
    result = str(
        html.xpath("//p[@class='d-review__average']/strong/text()")
        [0]).replace('\\n', '').replace('\n', '').replace('点', '')
    return result
