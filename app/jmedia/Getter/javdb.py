...
import json
from app.jmedia.Function.getHtml import get_html_javdb
...
def getStudio(htmlcode):
    html = etree.fromstring(htmlcode,
                            etree.HTMLParser())  # //table/tr[1]/td[1]/text()
    result1 = str(
        html.xpath('//strong[contains(text(),"片商:")]/../span/a/text()')).strip(
            " ['']")
    result2 = str(
        html.xpath('//strong[contains(text(),"Maker:")]/../span/a/text()')
    ).strip(" ['']")
...
def getPublisher(htmlcode):
    html = etree.fromstring(htmlcode,
                            etree.HTMLParser())  # //table/tr[1]/td[1]/text()
    result1 = str(
        html.xpath('//strong[contains(text(),"發行:")]/../span/a/text()')).strip(
            " ['']")
    result2 = str(
        html.xpath('//strong[contains(text(),"Publisher:")]/../span/a/text()')
    ).strip(" ['']")
    return str(result1 + result2).strip('+').replace("', '",
                                                     '').replace('"', '')
...
def getRuntime(htmlcode):
    ...
    result2 = str(
        html.xpath('//strong[contains(text(),"Duration:")]/../span/text()')
    ).strip(" ['']")
...
def getSeries(htmlcode):
    html = etree.fromstring(htmlcode,
                            etree.HTMLParser())  # //table/tr[1]/td[1]/text()
    result1 = str(
        html.xpath('//strong[contains(text(),"系列:")]/../span/a/text()')).strip(
            " ['']")
    result2 = str(
        html.xpath('//strong[contains(text(),"Series:")]/../span/a/text()')
    ).strip(" ['']")
    return str(result1 + result2).strip('+').replace("', '",
                                                     '').replace('"', '')
...
def getNumber(htmlcode):
    html = etree.fromstring(htmlcode, etree.HTMLParser())
    result1 = str(
        html.xpath('//strong[contains(text(),"番號:")]/../span/a/text()')).strip(
            " ['']").replace('_', '-')
    result2 = str(
        html.xpath('//strong[contains(text(),"ID:")]/../span/a/text()')).strip(
            " ['']").replace('_', '-')
...
def getRelease(htmlcode):
    ...
    result2 = str(
        html.xpath('//strong[contains(text(),"Released Date:")]/../span/text()'
                   )).strip(" ['']")
...
def getTag(htmlcode):
    html = etree.fromstring(htmlcode,
                            etree.HTMLParser())  # //table/tr[1]/td[1]/text()
    result1 = str(
        html.xpath('//strong[contains(text(),"類別:")]/../span/a/text()')).strip(
            " ['']")
    result2 = str(
        html.xpath('//strong[contains(text(),"Tags:")]/../span/a/text()')
    ).strip(" ['']")
    return str(result1 + result2).strip('+').replace(",\\xa0", "").replace(
...
def getExtraFanart(htmlcode):  # 获取封面链接
    ...
    extrafanart_list = html.xpath(
        "//div[@class='message-body']/div[@class='tile-images preview-images']/a/@href"
    )
...
def getDirector(htmlcode):
    html = etree.fromstring(htmlcode,
                            etree.HTMLParser())  # //table/tr[1]/td[1]/text()
    result1 = str(
        html.xpath('//strong[contains(text(),"導演:")]/../span/a/text()')).strip(
            " ['']")
    result2 = str(
        html.xpath('//strong[contains(text(),"Director:")]/../span/a/text()')
    ).strip(" ['']")
    return str(result1 + result2).strip('+').replace("', '",
                                                     '').replace('"', '')
...
def getScore(htmlcode):
    html = etree.fromstring(htmlcode,
                            etree.HTMLParser())  # //table/tr[1]/td[1]/text()
    result = str(
        html.xpath("//span[@class='score-label']/text()")).strip(" ['']")
    score = 0
...
def getOutlineScore(number):  # 获取简介
    ...
    score = ''
    try:
        response = post_html("https://www.jav321.com/search",
                             query={"sn": number})
        ...
        if re.search(r'<b>平均評価</b>: <img data-original="/img/(\d+).gif" />',
                     response):
            score = re.findall(
                r'<b>平均評価</b>: <img data-original="/img/(\d+).gif" />',
                response)[0]
            score = str(float(score) / 10.0)
        else:
            score = str(re.findall(r'<b>平均評価</b>: ([^<]+)<br>',
...
def main(number, appoint_url, isuncensored=False):
    try:
        if appoint_url == '':
            # ========================================================================搜索番号
            htmlcode = get_html_javdb('https://javdb.com/search?q=' + number +
        ...
        # ========================================================================请求、判断结果
        html_info = get_html_javdb(result_url + '?locale=zh').replace(
            u'\xa0', u' ')
        ...
        imagecut = 1
        outline = ''
        if isuncensored and (re.match('^\d{4,}', number)
                             or re.match('n\d{4}', number)):  # 无码，收集封面、评分
            imagecut = 0
        ...
        dic = {
            ...
            getTitle(html_info).replace('中文字幕', '').replace('無碼', '').replace(
                "\\n",
                '').replace('_', '-').replace(number.upper(), '').replace(
