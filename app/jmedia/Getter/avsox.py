...
def getTitle(a):
    try:
        ...
        result = str(html.xpath('/html/body/div[2]/h3/text()')).strip(
            " ['']")  # [0]
...
def getNum(a):
    html = etree.fromstring(a,
                            etree.HTMLParser())  # //table/tr[1]/td[1]/text()
    result1 = str(
        html.xpath('//span[contains(text(),"识别码:")]/../span[2]/text()')).strip(
            " ['']")
    return result1
...
def getCover_small(htmlcode, count):
    html = etree.fromstring(htmlcode, etree.HTMLParser())
    cover_small = html.xpath("//div[@id='waterfall']/div[" + str(count) +
