...
class DbHelper:
    ...
    def get_search_result_by_id(self, dl_id):
        ...
        """
        return self._db.query(SEARCHRESULTINFO).filter(
            SEARCHRESULTINFO.ID == dl_id).all()
    ...
    def is_torrent_rssd(self, enclosure):
        ...
        if not enclosure:
            return True
        if self._db.query(RSSTORRENTS).filter(
                RSSTORRENTS.ENCLOSURE == enclosure).count() > 0:
            return True
        else:
            return False
    ...
    def is_userrss_finished(self, torrent_name, enclosure):
        ...
        if not torrent_name and not enclosure:
            return True
        if enclosure:
            ret = self._db.query(RSSTORRENTS).filter(
                RSSTORRENTS.ENCLOSURE == enclosure).count()
        else:
            ret = self._db.query(RSSTORRENTS).filter(
                RSSTORRENTS.TORRENT_NAME == torrent_name).count()
    ...
    def insert_rss_torrents(self, media_info):
        ...
        """
        self._db.insert(
            RSSTORRENTS(TORRENT_NAME=media_info.org_string,
    ...
    def simple_delete_rss_torrents(self, title, enclosure):
        ...
        if enclosure:
            ...
            self._db.query(RSSTORRENTS).filter(
                RSSTORRENTS.TORRENT_NAME == title).delete()
    ...
    def is_douban_media_exists(self, media):
        ...
        if not media:
            return True
        if self._db.query(DOUBANMEDIAS).filter(
                DOUBANMEDIAS.NAME == media.get_name()).count() > 0:
            return True
        else:
            return False
    ...
    def insert_douban_media_state(self, media, state):
        ...
        if self.is_douban_media_exists(media):
            ...
            # 插入
            self._db.insert(
                DOUBANMEDIAS(NAME=media.get_name(),
                             YEAR=media.year,
    ...
    def update_douban_media_state(self, media, state):
        ...
        self._db.query(DOUBANMEDIAS).filter(
            DOUBANMEDIAS.NAME == media.title,
            DOUBANMEDIAS.YEAR == media.year).update({"STATE": state})
    ...
    def get_douban_search_state(self, title, year=None):
        ...
        if not year:
            return self._db.query(
                DOUBANMEDIAS.STATE).filter(DOUBANMEDIAS.NAME == title).first()
        else:
            return self._db.query(DOUBANMEDIAS.STATE).filter(
                DOUBANMEDIAS.NAME == title,
                DOUBANMEDIAS.YEAR == str(year)).first()

    def is_transfer_history_exists(self, source_path, source_filename,
    ...
    @DbPersist(_db)
    def insert_transfer_history(self, in_from: Enum, rmt_mode: RmtMode,
                                in_path, out_path, dest, media_info):
        """
        插入识别转移记录
        """
        if not media_info:
        ...
        timestr = time.strftime('%Y-%m-%d %H:%M:%S',
                                time.localtime(time.time()))
        self._db.insert(
            TRANSFERHISTORY(MODE=str(rmt_mode.value),
                            TYPE=media_info.type.value,
        ...
        if search:
            ...
            data = self._db.query(TRANSFERHISTORY).filter(
                ...
                | (TRANSFERHISTORY.TITLE.like(search))).order_by(
                    TRANSFERHISTORY.DATE.desc()).limit(
                        int(rownum)).offset(begin_pos).all()
            return count, data
        else:
            return self._db.query(TRANSFERHISTORY).count(), self._db.query(
                TRANSFERHISTORY).order_by(TRANSFERHISTORY.DATE.desc()).limit(
                    int(rownum)).offset(begin_pos).all()
    ...
    def get_transfer_path_by_id(self, logid):
        ...
        return self._db.query(TRANSFERHISTORY).filter(
            TRANSFERHISTORY.ID == int(logid)).all()
    ...
    def delete_transfer_log_by_id(self, logid):
        ...
        self._db.query(TRANSFERHISTORY).filter(
            TRANSFERHISTORY.ID == int(logid)).delete()

    def get_transfer_unknown_paths(self, ):
        """
        查询未识别的记录列表
        """
        return self._db.query(TRANSFERUNKNOWN).filter(
            TRANSFERUNKNOWN.STATE == 'N').all()
    ...
    def update_transfer_unknown_state(self, path):
        ...
        self._db.query(TRANSFERUNKNOWN).filter(
            TRANSFERUNKNOWN.PATH == os.path.normpath(path)).update(
                {"STATE": "Y"})
    ...
    def delete_transfer_unknown(self, tid):
        ...
        if not tid:
            return []
        self._db.query(TRANSFERUNKNOWN).filter(
            TRANSFERUNKNOWN.ID == int(tid)).delete()
    ...
    def get_unknown_path_by_id(self, tid):
        ...
        if not tid:
            return []
        return self._db.query(TRANSFERUNKNOWN).filter(
            TRANSFERUNKNOWN.ID == int(tid)).all()
    ...
    def is_transfer_unknown_exists(self, path):
        ...
        ret = self._db.query(TRANSFERUNKNOWN).filter(
            TRANSFERUNKNOWN.PATH == os.path.normpath(path)).count()
        if ret > 0:
            return True
        else:
            return False
    ...
    def is_transfer_in_blacklist(self, path):
        ...
        ret = self._db.query(TRANSFERBLACKLIST).filter(
            TRANSFERBLACKLIST.PATH == os.path.normpath(path)).count()
        if ret > 0:
            return True
        else:
            return False
    ...
    def get_config_site(self, ):
        ...
        查询所有站点信息
        """
        return self._db.query(CONFIGSITE).order_by(
            cast(CONFIGSITE.PRI, Integer).asc())

    def get_site_by_id(self, tid):
        """
        查询1个站点信息
        """
        return self._db.query(CONFIGSITE).filter(
            CONFIGSITE.ID == int(tid)).all()
    ...
    @DbPersist(_db)
    def insert_config_site(self, name, site_pri, rssurl, signurl, cookie, note,
                           rss_uses):
        """
        插入站点信息
        """
    ...
    @DbPersist(_db)
    def update_config_site(self, tid, name, site_pri, rssurl, signurl, cookie,
                           note, rss_uses):
        """
        更新站点信息
        """
    ...
    def update_site_cookie_ua(self, tid, cookie, ua=None):
        ...
        if not tid:
            return
        rec = self._db.query(CONFIGSITE).filter(
            CONFIGSITE.ID == int(tid)).first()
        if rec.NOTE:
    ...
    def get_config_filter_group(self, gid=None):
        ...
        """
        if gid:
            return self._db.query(CONFIGFILTERGROUP).filter(
                CONFIGFILTERGROUP.ID == int(gid)).all()
    ...
    def get_config_filter_rule(self, groupid=None):
        ...
        """
        if not groupid:
            return self._db.query(CONFIGFILTERRULES).order_by(
                CONFIGFILTERRULES.GROUP_ID,
                cast(CONFIGFILTERRULES.PRIORITY, Integer)).all()
        else:
            return self._db.query(CONFIGFILTERRULES).filter(
                CONFIGFILTERRULES.GROUP_ID == int(groupid)).order_by(
                    CONFIGFILTERRULES.GROUP_ID,
                    cast(CONFIGFILTERRULES.PRIORITY, Integer)).all()
    ...
    def get_rss_movies(self, state=None, rssid=None):
        ...
        if rssid:
            return self._db.query(RSSMOVIES).filter(
                RSSMOVIES.ID == int(rssid)).all()
        else:
            if not state:
                return self._db.query(RSSMOVIES).all()
            else:
                return self._db.query(RSSMOVIES).filter(
                    RSSMOVIES.STATE == state).all()
    ...
    def get_rss_movie_id(self, title, year=None, tmdbid=None):
        ...
        if tmdbid:
            ret = self._db.query(
                RSSMOVIES.ID).filter(RSSMOVIES.TMDBID == str(tmdbid)).first()
            if ret:
                return ret[0]
        if not year:
            items = self._db.query(RSSMOVIES).filter(
                RSSMOVIES.NAME == title).all()
        else:
            items = self._db.query(RSSMOVIES).filter(
                RSSMOVIES.NAME == title, RSSMOVIES.YEAR == str(year)).all()
        if items:
            if tmdbid:
                for item in items:
    ...
    def get_rss_movie_sites(self, rssid):
        ...
        if not rssid:
            return ""
        ret = self._db.query(
            RSSMOVIES.DESC).filter(RSSMOVIES.ID == int(rssid)).first()
        if ret:
            return ret[0]
        return ""

    @DbPersist(_db)
    def update_rss_movie_tmdb(self, rid, tmdbid, title, year, image, desc,
                              note):
        """
        更新订阅电影的部分信息
        """
    ...
    def update_rss_filter_order(self, rtype, rssid, res_order):
        ...
        if rtype == MediaType.MOVIE:
            self._db.query(RSSMOVIES).filter(
                RSSMOVIES.ID == int(rssid)).update({"FILTER_ORDER": res_order})
        else:
    ...
    def get_rss_overedition_order(self, rtype, rssid):
        ...
        """
        if rtype == MediaType.MOVIE:
            res = self._db.query(RSSMOVIES.FILTER_ORDER).filter(
                RSSMOVIES.ID == int(rssid)).first()
        else:
            res = self._db.query(
                RSSTVS.FILTER_ORDER).filter(RSSTVS.ID == int(rssid)).first()
        if res and res[0]:
            return int(res[0])
        else:
            return 0
    ...
    def is_exists_rss_movie(self, title, year):
        ...
        if not title:
            return False
        count = self._db.query(RSSMOVIES).filter(
            RSSMOVIES.NAME == title, RSSMOVIES.YEAR == str(year)).count()
        if count > 0:
            return True
        else:
            return False
    ...
    def insert_rss_movie(self,
        ...
        if self.is_exists_rss_movie(media_info.title, media_info.year):
            return 9
        self._db.insert(
            RSSMOVIES(NAME=media_info.title,
                      YEAR=media_info.year,
    ...
    def delete_rss_movie(self, title=None, year=None, rssid=None, tmdbid=None):
        ...
        if rssid:
            self._db.query(RSSMOVIES).filter(
                RSSMOVIES.ID == int(rssid)).delete()
        else:
            if tmdbid:
                self._db.query(RSSMOVIES).filter(
                    RSSMOVIES.TMDBID == tmdbid).delete()
            self._db.query(RSSMOVIES).filter(
                RSSMOVIES.NAME == title, RSSMOVIES.YEAR == str(year)).delete()

    @DbPersist(_db)
    def update_rss_movie_state(self,
                               title=None,
                               year=None,
                               rssid=None,
                               state='R'):
        ...
        if rssid:
            self._db.query(RSSMOVIES).filter(
                RSSMOVIES.ID == int(rssid)).update({"STATE": state})
        else:
            self._db.query(RSSMOVIES).filter(
    ...
    def get_rss_tvs(self, state=None, rssid=None):
        ...
        if rssid:
            ...
            if not state:
                ...
                return self._db.query(RSSTVS).filter(
                    RSSTVS.STATE == state).all()
    ...
    def get_rss_tv_id(self, title, year=None, season=None, tmdbid=None):
        ...
        if tmdbid:
            if season:
                ret = self._db.query(RSSTVS.ID).filter(
                    RSSTVS.TMDBID == tmdbid, RSSTVS.SEASON == season).first()
            else:
                ret = self._db.query(
                    RSSTVS.ID).filter(RSSTVS.TMDBID == tmdbid).first()
            if ret:
                return ret[0]
        if season and year:
            items = self._db.query(RSSTVS).filter(
                RSSTVS.NAME == title, RSSTVS.SEASON == str(season),
                RSSTVS.YEAR == str(year)).all()
        elif season and not year:
            items = self._db.query(RSSTVS).filter(
                RSSTVS.NAME == title, RSSTVS.SEASON == str(season)).all()
        elif not season and year:
            items = self._db.query(RSSTVS).filter(
                RSSTVS.NAME == title, RSSTVS.YEAR == str(year)).all()
        else:
    ...
    def is_exists_rss_tv(self, title, year, season=None):
        ...
        if not title:
            return False
        if season:
            count = self._db.query(RSSTVS).filter(
                RSSTVS.NAME == title, RSSTVS.YEAR == str(year),
                RSSTVS.SEASON == season).count()
        else:
            count = self._db.query(RSSTVS).filter(
                RSSTVS.NAME == title, RSSTVS.YEAR == str(year)).count()
        if count > 0:
            return True
        else:
            return False
    ...
    def insert_rss_tv(self,
        ...
        if self.is_exists_rss_tv(media_info.title, media_info.year,
                                 season_str):
            return 9
        self._db.insert(
            RSSTVS(NAME=media_info.title,
                   YEAR=media_info.year,
                   SEASON=season_str,
                   TMDBID=media_info.tmdb_id,
                   IMAGE=media_info.get_message_image(),
    ...
    @DbPersist(_db)
    def update_rss_tv_lack(self,
                           title=None,
                           year=None,
                           season=None,
                           rssid=None,
                           lack_episodes: list = None):
        """
        更新电视剧缺失的集数
        """
        if not title and not rssid:
    ...
    def delete_rss_tv(self, title=None, season=None, rssid=None, tmdbid=None):
        ...
        if not title and not rssid:
            return
        if not rssid:
            rssid = self.get_rss_tv_id(title=title,
                                       tmdbid=tmdbid,
                                       season=season)
        if rssid:
    ...
    def is_exists_rss_tv_episodes(self, rid):
        ...
        if not rid:
            return False
        count = self._db.query(RSSTVEPISODES).filter(
            RSSTVEPISODES.RSSID == int(rid)).count()
        if count > 0:
            return True
        else:
            return False
    ...
    def update_rss_tv_episodes(self, rid, episodes):
        ...
        if self.is_exists_rss_tv_episodes(rid):
            self._db.query(RSSTVEPISODES).filter(
                RSSTVEPISODES.RSSID == int(rid)).update(
                    {"EPISODES": ",".join(episodes)})
        else:
            self._db.insert(
                RSSTVEPISODES(RSSID=rid, EPISODES=",".join(episodes)))
    ...
    def get_rss_tv_episodes(self, rid):
        ...
        if not rid:
            return []
        ret = self._db.query(
            RSSTVEPISODES.EPISODES).filter(RSSTVEPISODES.RSSID == rid).first()
        if ret:
    ...
    def delete_rss_tv_episodes(self, rid):
        ...
        if not rid:
            return
        self._db.query(RSSTVEPISODES).filter(
            RSSTVEPISODES.RSSID == int(rid)).delete()

    @DbPersist(_db)
    def update_rss_tv_state(self,
                            title=None,
                            year=None,
                            season=None,
                            rssid=None,
    ...
    def is_user_exists(self, name):
        ...
        if not name:
            return False
        count = self._db.query(CONFIGUSERS).filter(
            CONFIGUSERS.NAME == name).count()
        if count > 0:
            return True
        else:
            return False
    ...
    def insert_user(self, name, password, pris):
        ...
        if self.is_user_exists(name):
            ...
            self._db.insert(
                CONFIGUSERS(NAME=name, PASSWORD=password, PRIS=pris))
    ...
    def get_transfer_statistics(self, days=30):
        ...
        return self._db.query(
            ...
            func.count('*')).filter(
                TRANSFERHISTORY.DATE > begin_date).group_by(
                    func.substr(TRANSFERHISTORY.DATE, 1,
                                10)).order_by(TRANSFERHISTORY.DATE).all()
    ...
    def update_site_user_statistics_site_name(self, new_name, old_name):
        ...
        更新站点用户数据中站点名称
        """
        self._db.query(SITEUSERINFOSTATS).filter(
            SITEUSERINFOSTATS.SITE == old_name).update({"SITE": new_name})
    ...
    def update_site_user_statistics(self, site_user_infos: list):
        ...
        for site_user_info in site_user_infos:
            ...
            if not self.is_exists_site_user_statistics(url):
                ...
                self._db.query(SITEUSERINFOSTATS).filter(
                    SITEUSERINFOSTATS.URL == url).update({
                        "SITE":
                        site,
                        "USERNAME":
                        username,
    ...
    def is_exists_site_user_statistics(self, url):
        ...
        判断站点数据是滞存在
        """
        count = self._db.query(SITEUSERINFOSTATS).filter(
            SITEUSERINFOSTATS.URL == url).count()
        if count > 0:
            return True
        else:
            return False
        ...
        for site_user_info in site_user_infos:
            ...
            if not self.is_exists_site_favicon(site_user_info.site_name):
                ...
                self._db.query(SITEFAVICON).filter(
                    SITEFAVICON.SITE == site_user_info.site_name).update({
                        "URL":
    ...
    def is_exists_site_favicon(self, site):
        ...
        判断站点图标是否存在
        """
        count = self._db.query(SITEFAVICON).filter(
            SITEFAVICON.SITE == site).count()
        if count > 0:
            return True
        else:
            return False
    ...
    def get_site_favicons(self, site=None):
        ...
        """
        if site:
            return self._db.query(SITEFAVICON).filter(
                SITEFAVICON.SITE == site).all()
        else:
    ...
    def update_site_seed_info_site_name(self, new_name, old_name):
        ...
        self._db.query(SITEUSERSEEDINGINFO).filter(
            SITEUSERSEEDINGINFO.SITE == old_name).update({"SITE": new_name})
    ...
    def update_site_seed_info(self, site_user_infos: list):
        ...
        update_at = time.strftime('%Y-%m-%d %H:%M:%S',
                                  time.localtime(time.time()))
        for site_user_info in site_user_infos:
            if not self.is_site_seeding_info_exist(
                    url=site_user_info.site_url):
                self._db.insert(
                    SITEUSERSEEDINGINFO(
                        SITE=site_user_info.site_name,
                ...
                self._db.query(SITEUSERSEEDINGINFO).filter(
                    SITEUSERSEEDINGINFO.URL == site_user_info.site_url).update(
                        {
                            "SITE": site_user_info.site_name,
                            "UPDATE_AT": update_at,
                            "SEEDING_INFO": site_user_info.seeding_info
    ...
    def is_site_statistics_history_exists(self, url, date):
        ...
        if not url or not date:
            return False
        count = self._db.query(SITESTATISTICSHISTORY).filter(
    ...
    def update_site_statistics_site_name(self, new_name, old_name):
        ...
        self._db.query(SITESTATISTICSHISTORY).filter(
            SITESTATISTICSHISTORY.SITE == old_name).update({"SITE": new_name})
    ...
    def insert_site_statistics_history(self, site_user_infos: list):
        ...
        for site_user_info in site_user_infos:
            ...
            bonus = site_user_info.bonus
            url = site_user_info.site_url
            if not self.is_site_statistics_history_exists(date=date_now,
                                                          url=url):
                self._db.insert(
    ...
    def get_site_statistics_history(self, site, days=30):
        ...
        return self._db.query(SITESTATISTICSHISTORY).filter(
            SITESTATISTICSHISTORY.SITE == site).order_by(
                SITESTATISTICSHISTORY.DATE.asc()).limit(days)
        ...
        if date_ret and date_ret[0][0]:
            ...
            if strict_urls:
                subquery = self._db.query(
                    ...
                    func.sum(SITESTATISTICSHISTORY.DOWNLOAD).label(
                        "DOWNLOAD")).filter(
                            SITESTATISTICSHISTORY.DATE >= min_date,
                            SITESTATISTICSHISTORY.URL.in_(
                                tuple(strict_urls + ["__DUMMY__"]))).group_by(
                                    SITESTATISTICSHISTORY.SITE,
                                    SITESTATISTICSHISTORY.DATE).subquery()
            else:
                subquery = self._db.query(
                    SITESTATISTICSHISTORY.SITE.label("SITE"),
                    SITESTATISTICSHISTORY.DATE.label("DATE"),
                ...
                ).filter(SITESTATISTICSHISTORY.DATE >= min_date).group_by(
                    SITESTATISTICSHISTORY.SITE,
                    SITESTATISTICSHISTORY.DATE).subquery()
            rets = self._db.query(subquery.c.SITE, func.min(subquery.c.UPLOAD),
                                  func.min(subquery.c.DOWNLOAD),
                                  func.max(subquery.c.UPLOAD),
                                  func.max(subquery.c.DOWNLOAD)).group_by(
                                      subquery.c.SITE).all()
            ret_sites = []
            for ret_b in rets:
    ...
    def is_exists_download_history(self, title, tmdbid, mtype=None):
        ...
        if mtype:
            ...
            count = self._db.query(DOWNLOADHISTORY).filter(
                (DOWNLOADHISTORY.TITLE == title)
                | (DOWNLOADHISTORY.TMDBID == tmdbid)).count()
        if count > 0:
            return True
        else:
            return False
    ...
    def insert_download_history(self, media_info):
        ...
        if not media_info.title or not media_info.tmdb_id:
            return
        if self.is_exists_download_history(media_info.title,
                                           media_info.tmdb_id,
    ...
    def get_download_history(self, date=None, hid=None, num=30, page=1):
        ...
        if hid:
            return self._db.query(DOWNLOADHISTORY).filter(
                DOWNLOADHISTORY.ID == int(hid)).all()
        elif date:
            return self._db.query(DOWNLOADHISTORY).filter(
                DOWNLOADHISTORY.DATE > date).order_by(
                    DOWNLOADHISTORY.DATE.desc()).all()
        else:
    ...
    def is_media_downloaded(self, title, tmdbid):
        ...
        if self.is_exists_download_history(title, tmdbid):
            return True
        count = self._db.query(TRANSFERHISTORY).filter(
            TRANSFERHISTORY.TITLE == title).count()
        if count > 0:
            return True
        else:
            return False
    ...
    def insert_brushtask(self, brush_id, item):
        ...
        if not brush_id:
            self._db.insert(
                SITEBRUSHTASK(NAME=item.get('name'),
                              SITE=item.get('site'),
                              ...
                              LST_MOD_DATE=time.strftime(
                                  '%Y-%m-%d %H:%M:%S',
                                  time.localtime(time.time())),
                              SENDMESSAGE=item.get('sendmessage'),
                              FORCEUPLOAD=item.get('forceupload')))
        else:
            self._db.query(SITEBRUSHTASK).filter(
                SITEBRUSHTASK.ID == int(brush_id)).update({
                    "NAME":
                    item.get('name'),
                    "SITE":
                    item.get('site'),
    ...
    def delete_brushtask(self, brush_id):
        ...
        """
        self._db.query(SITEBRUSHTASK).filter(
            SITEBRUSHTASK.ID == int(brush_id)).delete()
        self._db.query(SITEBRUSHTORRENTS).filter(
            SITEBRUSHTORRENTS.TASK_ID == brush_id).delete()
    ...
    def get_brushtasks(self, brush_id=None):
        ...
        """
        if brush_id:
            return self._db.query(SITEBRUSHTASK).filter(
                SITEBRUSHTASK.ID == int(brush_id)).first()
        else:
            # 根据站点优先级排序
    ...
    def get_brushtask_totalsize(self, brush_id):
        ...
        if not brush_id:
            return 0
        ret = self._db.query(
            func.sum(cast(SITEBRUSHTORRENTS.TORRENT_SIZE, Integer))).filter(
    ...
    def add_brushtask_download_count(self, brush_id):
        ...
        self._db.query(SITEBRUSHTASK).filter(
            SITEBRUSHTASK.ID == int(brush_id)).update({
                "DOWNLOAD_COUNT":
                SITEBRUSHTASK.DOWNLOAD_COUNT + 1,
                "LST_MOD_DATE":
    ...
    @DbPersist(_db)
    def add_brushtask_upload_count(self, brush_id, upload_size, download_size,
                                   remove_count):
        """
        更新上传下载量和删除种子数
        """
        ...
        self._db.query(SITEBRUSHTASK).filter(
            SITEBRUSHTASK.ID == int(brush_id)).update({
                "REMOVE_COUNT":
                SITEBRUSHTASK.REMOVE_COUNT + remove_count,
                "UPLOAD_SIZE":
                int(upload_size) + delete_upsize,
                "DOWNLOAD_SIZE":
                int(download_size) + delete_dlsize,
            })

    @DbPersist(_db)
    def insert_brushtask_torrent(self, brush_id, title, enclosure, downloader,
                                 download_id, size):
        """
        增加刷流下载的种子信息
        """
    ...
    def get_brushtask_torrents(self, brush_id, active=True):
        ...
        if active:
            ...
            return self._db.query(SITEBRUSHTORRENTS).filter(
                SITEBRUSHTORRENTS.TASK_ID == int(brush_id)).order_by(
                    SITEBRUSHTORRENTS.LST_MOD_DATE.desc()).all()
    ...
    def get_user_downloaders(self, did=None):
        ...
        """
        if did:
            return self._db.query(SITEBRUSHDOWNLOADERS).filter(
                SITEBRUSHDOWNLOADERS.ID == int(did)).first()
        else:
    ...
    def update_user_downloader(self, did, name, dtype, user_config, note):
        ...
        """
        if did:
            self._db.query(SITEBRUSHDOWNLOADERS).filter(
                SITEBRUSHDOWNLOADERS.ID == int(did)).update({
                    "NAME":
                    name,
                    "TYPE":
                    dtype,
                    "HOST":
                    user_config.get("host"),
                    "PORT":
                    ...
                    "SAVE_DIR":
                    user_config.get("save_dir"),
                    "NOTE":
                    note
                })
        else:
            self._db.insert(
                SITEBRUSHDOWNLOADERS(NAME=name,
                                     TYPE=dtype,
    ...
    def delete_user_downloader(self, did):
        ...
        """
        self._db.query(SITEBRUSHDOWNLOADERS).filter(
            SITEBRUSHDOWNLOADERS.ID == int(did)).delete()

    @DbPersist(_db)
    def add_filter_group(self, name, default='N', type=''):
        """
        新增规则组
        """
        if default == 'Y':
        ...
        if group_id:
            self._db.query(CONFIGFILTERGROUP).filter(
                CONFIGFILTERGROUP.ID == int(group_id)).update(
                    {"IS_DEFAULT": default})
        else:
            self._db.insert(
                CONFIGFILTERGROUP(GROUP_NAME=name,
                                  IS_DEFAULT=default,
                                  TYPE=type))

    def get_filter_groupid_by_name(self, name):
        ret = self._db.query(CONFIGFILTERGROUP.ID).filter(
            CONFIGFILTERGROUP.GROUP_NAME == name).first()
        if ret:
            return ret[0]
        else:
            return ""
    ...
    def set_default_filtergroup(self, groupid):
        ...
        """
        self._db.query(CONFIGFILTERGROUP).filter(
            CONFIGFILTERGROUP.ID == int(groupid)).update({"IS_DEFAULT": 'Y'})
        self._db.query(CONFIGFILTERGROUP).filter(
            CONFIGFILTERGROUP.ID != int(groupid)).update({"IS_DEFAULT": 'N'})
    ...
    def delete_filtergroup(self, groupid):
        ...
        删除规则组
        """
        self._db.query(CONFIGFILTERRULES).filter(
            CONFIGFILTERRULES.GROUP_ID == groupid).delete()
        self._db.query(CONFIGFILTERGROUP).filter(
            CONFIGFILTERGROUP.ID == int(groupid)).delete()
    ...
    def delete_filterrule(self, ruleid):
        ...
        """
        self._db.query(CONFIGFILTERRULES).filter(
            CONFIGFILTERRULES.ID == int(ruleid)).delete()
    ...
    def insert_filter_rule(self, item, ruleid=None):
        ...
        if ruleid:
            self._db.query(CONFIGFILTERRULES).filter(
                CONFIGFILTERRULES.ID == int(ruleid)).update({
                    "ROLE_NAME":
                    item.get("name"),
                    "PRIORITY":
                    item.get("pri"),
            ...
            self._db.insert(
                CONFIGFILTERRULES(GROUP_ID=item.get("group"),
    ...
    def get_userrss_tasks(self, tid=None):
        if tid:
            return self._db.query(CONFIGUSERRSS).filter(
                CONFIGUSERRSS.ID == int(tid)).all()
        else:
            return self._db.query(CONFIGUSERRSS).order_by(
                CONFIGUSERRSS.STATE.desc()).all()

    @DbPersist(_db)
    def delete_userrss_task(self, tid):
        if not tid:
            return
        self._db.query(CONFIGUSERRSS).filter(
            CONFIGUSERRSS.ID == int(tid)).delete()
    ...
    def update_userrss_task_info(self, tid, count):
        if not tid:
            return
        self._db.query(CONFIGUSERRSS).filter(
            CONFIGUSERRSS.ID == int(tid)).update({
                "PROCESS_COUNT":
                CONFIGUSERRSS.PROCESS_COUNT + count,
                "UPDATE_TIME":
                time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time()))
            })
    ...
    def update_userrss_task(self, item):
        if item.get("id") and self.get_userrss_tasks(item.get("id")):
            self._db.query(CONFIGUSERRSS).filter(
                CONFIGUSERRSS.ID == int(item.get("id"))).update({
                    "NAME":
                    item.get("name"),
                    "ADDRESS":
                    item.get("address"),
                    ...
                    item.get("save_path"),
                    "DOWNLOAD_SETTING":
                    item.get("download_setting"),
                    "RECOGNIZATION":
                    item.get("recognization"),
                    "OVER_EDITION":
                    int(item.get("over_edition")) if str(
                        item.get("over_edition")).isdigit() else 0,
                    "SITES":
                    json.dumps(item.get("sites")),
                    "FILTER_ARGS":
                    json.dumps(item.get("filter_args")),
                    "NOTE":
                    ""
                })
        else:
            self._db.insert(
                CONFIGUSERRSS(NAME=item.get("name"),
    ...
    def insert_userrss_mediainfos(self, tid=None, mediainfo=None):
        if not tid or not mediainfo:
            return
        taskinfo = self._db.query(CONFIGUSERRSS).filter(
            CONFIGUSERRSS.ID == int(tid)).all()
        if not taskinfo:
            return
        mediainfos = json.loads(
            taskinfo[0].MEDIAINFOS) if taskinfo[0].MEDIAINFOS else []
    ...
    def get_userrss_parser(self, pid=None):
        if pid:
            return self._db.query(CONFIGRSSPARSER).filter(
                CONFIGRSSPARSER.ID == int(pid)).first()
        else:
    ...
    def delete_userrss_parser(self, pid):
        ...
        self._db.query(CONFIGRSSPARSER).filter(
            CONFIGRSSPARSER.ID == int(pid)).delete()
    ...
    def update_userrss_parser(self, item):
        ...
        if item.get("id") and self.get_userrss_parser(item.get("id")):
            self._db.query(CONFIGRSSPARSER).filter(
                CONFIGRSSPARSER.ID == int(item.get("id"))).update({
                    "NAME":
                    item.get("name"),
                    "TYPE":
                    item.get("type"),
            ...
            self._db.insert(
                CONFIGRSSPARSER(NAME=item.get("name"),
                                TYPE=item.get("type"),
    ...
    def insert_userrss_task_history(self, task_id, title, downloader):
        ...
        """
        self._db.insert(
            USERRSSTASKHISTORY(TASK_ID=task_id,
                               TITLE=title,
                               DOWNLOADER=downloader,
                               DATE=time.strftime('%Y-%m-%d %H:%M:%S',
                                                  time.localtime(
    ...
    def get_rss_history(self, rtype=None, rid=None):
        ...
        """
        if rid:
            return self._db.query(RSSHISTORY).filter(
                RSSHISTORY.ID == int(rid)).all()
        elif rtype:
            return self._db.query(RSSHISTORY).filter(RSSHISTORY.TYPE == rtype) \
                .order_by(RSSHISTORY.FINISH_TIME.desc()).all()
        return self._db.query(RSSHISTORY).order_by(
    ...
    def is_custom_words_existed(self,
                                gid=None,
                                replaced=None,
                                front=None,
                                back=None):
        ...
        """
        if replaced:
            count = self._db.query(CUSTOMWORDS).filter(
                CUSTOMWORDS.GROUP_ID == gid,
                CUSTOMWORDS.REPLACED == replaced).count()
        elif front and back:
            count = self._db.query(CUSTOMWORDS).filter(
                CUSTOMWORDS.GROUP_ID == gid, CUSTOMWORDS.FRONT == front,
    ...
    @DbPersist(_db)
    def insert_custom_word_groups(self,
                                  title,
                                  year,
                                  gtype,
                                  tmdbid,
    ...
    def delete_custom_word_group(self, gid):
        ...
        if not gid:
            return
        self._db.query(CUSTOMWORDS).filter(
            CUSTOMWORDS.GROUP_ID == int(gid)).delete()
        self._db.query(CUSTOMWORDGROUPS).filter(
            CUSTOMWORDGROUPS.ID == int(gid)).delete()
    ...
    def get_custom_word_groups(self, gid=None, tmdbid=None, gtype=None):
        ...
        """
        if gid:
            return self._db.query(CUSTOMWORDGROUPS).filter(
                CUSTOMWORDGROUPS.ID == int(gid)).all()
        if tmdbid and gtype:
            return self._db.query(CUSTOMWORDGROUPS).filter(
                CUSTOMWORDGROUPS.TMDBID == int(tmdbid),
    ...
    def is_custom_word_group_existed(self, tmdbid=None, gtype=None):
        ...
        if not gtype or not tmdbid:
            return False
        count = self._db.query(CUSTOMWORDGROUPS).filter(
            CUSTOMWORDGROUPS.TMDBID == int(tmdbid),
    ...
    def is_custom_word_group_title_existed(self, title=None, gtype=None):
        """
        查询自定义识别词组标题是否存在
        """
        if not gtype or not title:
            return False
        count = self._db.query(CUSTOMWORDGROUPS).filter(
            CUSTOMWORDGROUPS.TITLE == str(title),
            CUSTOMWORDGROUPS.TYPE == int(gtype)).count()
        if count > 0:
            return True
        else:
    ...
    @DbPersist(_db)
    def insert_config_sync_path(self,
                                source,
                                dest,
                                unknown,
                                mode,
        ...
        增加目录同步
        """
        rgId = ruleGroupId if ruleGroupId else 0
        cwId = customWordId if customWordId else 0
        return self._db.insert(
            CONFIGSYNCPATHS(SOURCE=source,
                            DEST=dest,
                            UNKNOWN=unknown,
                            MODE=mode,
                            RENAME=int(rename),
    ...
    def delete_config_sync_path(self, sid):
        ...
        if not sid:
            return
        self._db.query(CONFIGSYNCPATHS).filter(
            CONFIGSYNCPATHS.ID == int(sid)).delete()
    ...
    def get_config_sync_paths(self, sid=None):
        ...
        """
        if sid:
            return self._db.query(CONFIGSYNCPATHS).filter(
                CONFIGSYNCPATHS.ID == int(sid)).all()
        return self._db.query(CONFIGSYNCPATHS).all()

    @DbPersist(_db)
    def check_config_sync_paths(self,
                                sid=None,
                                source=None,
                                rename=None,
                                enabled=None):
        ...
        if sid and rename is not None:
            self._db.query(CONFIGSYNCPATHS).filter(
                CONFIGSYNCPATHS.ID == int(sid)).update({"RENAME": int(rename)})
        elif sid and enabled is not None:
            self._db.query(CONFIGSYNCPATHS).filter(
                CONFIGSYNCPATHS.ID == int(sid)).update(
                    {"ENABLED": int(enabled)})
        elif source and enabled is not None:
            self._db.query(CONFIGSYNCPATHS).filter(
                CONFIGSYNCPATHS.SOURCE == source).update(
                    {"ENABLED": int(enabled)})
    ...
    def delete_download_setting(self, sid):
        ...
        if not sid:
            return
        self._db.query(DOWNLOADSETTING).filter(
            DOWNLOADSETTING.ID == int(sid)).delete()
    ...
    def get_download_setting(self, sid=None):
        ...
        """
        if sid:
            return self._db.query(DOWNLOADSETTING).filter(
                DOWNLOADSETTING.ID == int(sid)).all()
        return self._db.query(DOWNLOADSETTING).all()

    @DbPersist(_db)
    def update_download_setting(self, sid, name, category, tags,
                                content_layout, is_paused, upload_limit,
                                download_limit, ratio_limit,
        ...
        if sid:
            self._db.query(DOWNLOADSETTING).filter(
                DOWNLOADSETTING.ID == int(sid)).update({
                    "NAME":
                    name,
                    "CATEGORY":
                    category,
                    ...
                    int(is_paused),
                    "UPLOAD_LIMIT":
                    int(float(upload_limit)),
                    "DOWNLOAD_LIMIT":
                    int(float(download_limit)),
                    "RATIO_LIMIT":
                    int(round(float(ratio_limit), 2) * 100),
                    "SEEDING_TIME_LIMIT":
                    int(float(seeding_time_limit)),
                    "DOWNLOADER":
                    downloader
                })
        else:
    ...
    def delete_message_client(self, cid):
        ...
        self._db.query(MESSAGECLIENT).filter(
            MESSAGECLIENT.ID == int(cid)).delete()
    ...
    def get_message_client(self, cid=None):
        ...
        """
        if cid:
            return self._db.query(MESSAGECLIENT).filter(
                MESSAGECLIENT.ID == int(cid)).all()
    ...
    def check_message_client(self,
        ...
        if cid and interactive is not None:
            self._db.query(MESSAGECLIENT).filter(
                MESSAGECLIENT.ID == int(cid)).update(
                    {"INTERACTIVE": int(interactive)})
        elif cid and enabled is not None:
            self._db.query(MESSAGECLIENT).filter(
                MESSAGECLIENT.ID == int(cid)).update({"ENABLED": int(enabled)})
        elif not cid and int(interactive) == 0 and ctype:
            self._db.query(MESSAGECLIENT).filter(
                MESSAGECLIENT.INTERACTIVE == 1,
                MESSAGECLIENT.TYPE == ctype).update({"INTERACTIVE": 0})
    ...
    def delete_torrent_remove_task(self, tid):
        ...
        if not tid:
            return
        self._db.query(TORRENTREMOVETASK).filter(
            TORRENTREMOVETASK.ID == int(tid)).delete()
    ...
    def get_torrent_remove_tasks(self, tid=None):
        ...
        """
        if tid:
            return self._db.query(TORRENTREMOVETASK).filter(
                TORRENTREMOVETASK.ID == int(tid)).all()
        return self._db.query(TORRENTREMOVETASK).order_by(
            TORRENTREMOVETASK.NAME).all()
    ...
    def delete_douban_history(self, hid):
        ...
        if not hid:
            return
        self._db.query(DOUBANMEDIAS).filter(
            DOUBANMEDIAS.ID == int(hid)).delete()

    def get_douban_history(self):
        """
        查询豆瓣同步记录
        """
        return self._db.query(DOUBANMEDIAS).order_by(
            DOUBANMEDIAS.ADD_TIME.desc()).all()
